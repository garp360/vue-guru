  ...
  getDeadlines: function(seasonData) {
    ...
    for (let week = 1; week <= 17; week++) {
      ...
      if (weeklyMatchups.length > 0) {
        ...
        let earliestGameTime = moment(
          weeklyMatchups[0].eid.substring(0, weeklyMatchups[0].eid.length - 2),
          'YYYYMMDD'
        )
        let latestGameTime = moment(
          weeklyMatchups[weeklyMatchups.length - 1].eid.substring(
            0,
            weeklyMatchups[weeklyMatchups.length - 1].eid.length - 2
          ),
          'YYYYMMDD'
        )
        ...
        let dline = moment(earliestGameTime)
          .weekday(7)
          .hour(11)
          .minute(45)
